import { PrismaClient, Role, StatutPoste } from '../src/generated';
...
async function main() {
  ...
  const admin = await prisma.user.upsert({
    ...
    update: { role: Role.ADMIN, isActive: true, email: 'admin@cybercontrol.local' },
    create: {
      username: 'admin',
      email: 'admin@cybercontrol.local',
  ...
  const staff = await prisma.user.upsert({
    ...
    update: { role: Role.STAFF, isActive: true, email: 'staff@cybercontrol.local' },
    create: {
      username: 'staff',
      email: 'staff@cybercontrol.local',
  ...
  for (let poste = 1; poste <= cyber.nombrePostes; poste++) {
    ...
    await prisma.postePresence.upsert({
      where: {
        cyberId_numeroPoste: { cyberId: cyber.id, numeroPoste: poste },
      },
      update: {},
